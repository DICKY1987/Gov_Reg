...
from govreg_core.P_01260207233100000010_canonical_hash import hash_canonical_data
from src.P_01999000042260124484_geu_governance.P_01999000042260124490_io_utils import read_json, write_json
from REGISTRY.apply_patch import apply_patch
    ...
    def _build_patch(self, mapping: Dict, capabilities: Dict, registry: Dict) -> Tuple[List[Dict], Dict]:
        ...
        for mapping_rec in sorted(mapping.get("mappings", []), key=lambda m: m["file"]):
            ...
            if file_path not in path_to_index:
                raise ValueError(f"Mapping file not found in registry: {file_path}")
