...
def main():
    ...
    governed_root = Path(__file__).resolve().parents[3]
    try:
        ...
        git_root = governed_root

    print(f"Project root: {governed_root}")
    ...
    # Pre-commit hook
    pre_commit_src = governed_root / "ID" / "2_cli_tools" / "hooks" / "P_01999000042260125106_pre_commit_dir_id_check.py"
...
source_path = Path({str(pre_commit_src)!r})
...
"""
    ...
    # Pre-push hook
    pre_push_src = governed_root / "ID" / "2_cli_tools" / "hooks" / "P_01999000042260125107_pre_push_governance_check.py"
...
source_path = Path({str(pre_push_src)!r})
spec = importlib.util.spec_from_file_location(source_path.stem, source_path)
module = importlib.util.module_from_spec(spec)
assert spec is not None and spec.loader is not None
spec.loader.exec_module(module)
...
"""
    ...
    for dir_path in evidence_dirs:
        full_path = governed_root / dir_path
    ...
    config_path = governed_root / "automation_config.json"
    if not config_path.exists():
        import json
        
        config = {
            "project_root": str(governed_root),
    ...
    print(f"     python {governed_root / 'ID' / '2_cli_tools' / 'maintenance' / 'P_01999000042260125113_healthcheck.py'} --config automation_config.json")
    print("  3. Run reconciliation:")
    print(f"     python {governed_root / 'ID' / '7_automation' / 'P_01999000042260125106_registry_filesystem_reconciler.py'} --project-root {governed_root} --registry {governed_root / '01999000042260124503_REGISTRY_file.json'}")
    print("  4. Start watcher (optional):")
    print(f"     python {governed_root / 'ID' / '1_runtime' / 'watchers' / 'P_01999000042260125105_dir_id_watcher.py'} --config automation_config.json")
    print()
