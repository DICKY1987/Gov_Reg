...
class SyncValidator:
    ...
    def __init__(self, scan_csv_path: str, registry_path: Optional[str] = None):
        """
        Initialize sync validator.
        
        Args:
            scan_csv_path: Path to file scan CSV
            registry_path: Path to registry (defaults to UNIFIED_SSOT_REGISTRY.json)
        """
        self.scan_csv_path = Path(scan_csv_path)
        self.registry_path = Path(registry_path) if registry_path else REGISTRY_PATH
        self.registry = RegistryStore(str(self.registry_path))
