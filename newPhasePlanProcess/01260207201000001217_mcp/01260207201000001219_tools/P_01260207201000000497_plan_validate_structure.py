...
def handle(args: Dict[str, Any], tool_spec: Dict[str, Any], _config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    script_path = Path(__file__).resolve().parents[2] / "scripts" / "P_01260207233100000263_validate_structure.py"
    if not script_path.exists():
        return _error_result(tool_spec, run_id, resolved_evidence, write_evidence_flag, "EXEC_BINDING_MISSING", "structure validation script not found", args)
    ...
    try:
        ...
        return _error_result(tool_spec, run_id, resolved_evidence, write_evidence_flag, "TIMEOUT", "structure validation timed out", args)

    script_evidence = evidence_dir / "structure_validation.json"
    defects = []
    artifacts_emitted = []
    hashes = {}
    if script_evidence.exists():
        artifacts_emitted.append(str(script_evidence))
        hashes["structure_evidence_sha256"] = hashing.sha256_file(script_evidence)
        try:
            evidence_data = json.loads(script_evidence.read_text(encoding="utf-8"))
            defects = defect_utils.from_structure_evidence(evidence_data)
            ...
            defects.append(
                {
                    ...
                    "message": "structure evidence JSON is invalid",
                }
            )

    status = "pass" if exit_code == 0 else "fail"
    summary = stdout.strip() or stderr.strip() or "plan structure validation complete"
...
def _error_result(
    ...
    defect = {
        "severity": "high",
        "code": code,
        "location": "mcp.plan.validate_structure",
        "message": message,
    }
    return build_envelope(
        tool=tool_spec["tool"],
