...
def handle(args: Dict[str, Any], tool_spec: Dict[str, Any], _config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    script_path = Path(__file__).resolve().parents[2] / "scripts" / "P_01260207233100000262_validate_step_contracts.py"
    if not script_path.exists():
        return _error_result(tool_spec, run_id, resolved_evidence, write_evidence_flag, "EXEC_BINDING_MISSING", "step contracts validation script not found", args)
    ...
    try:
        ...
        return _error_result(tool_spec, run_id, resolved_evidence, write_evidence_flag, "TIMEOUT", "step contracts validation timed out", args)

    script_evidence = evidence_dir / "step_contracts_validation.json"
    defects = []
    artifacts_emitted = []
    hashes = {}
    if script_evidence.exists():
        artifacts_emitted.append(str(script_evidence))
        hashes["step_contracts_evidence_sha256"] = hashing.sha256_file(script_evidence)
        try:
            evidence_data = json.loads(script_evidence.read_text(encoding="utf-8"))
            defects = defect_utils.from_step_contracts_evidence(evidence_data)
            ...
            defects.append(
                {
                    ...
                    "message": "step contracts evidence JSON is invalid",
                }
            )

    status = "pass" if exit_code == 0 else "fail"
    summary = stdout.strip() or stderr.strip() or "step contracts validation complete"
...
def _error_result(
    ...
    defect = {
        "severity": "high",
        "code": code,
        "location": "mcp.plan.validate_step_contracts",
        "message": message,
    }
    return build_envelope(
        tool=tool_spec["tool"],
