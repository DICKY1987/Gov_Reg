#!/usr/bin/env python3
# DOC_LINK: DOC-CORE-PHASE-PLAN-TO-WORKSTREAM-CONVERTER-PY-001
"""
...
"""
# DOC_ID: DOC-CORE-PHASE-PLAN-TO-WORKSTREAM-CONVERTER-PY-001
...
PLANS_DIR = REPO_ROOT / "contracts" / "phase_plans"
WORKSTREAMS_DIR = REPO_ROOT / "contracts" / "workstreams" / "generated"
...
def convert_to_workstream(phase_plan: Dict[str, Any]) -> Dict[str, Any]:
    ...
    workstream = {
        ...
        "file_id": phase_plan.get("file_id", ""),
        ...
        "tool_profiles_path": "orchestrator/config/ai_tool_profiles.json",
        "circuit_breakers_path": "orchestrator/config/circuit_breaker_rules.yaml",
...
def main() -> None:
    """Convert all phase plans in contracts/phase_plans to workstream JSON files."""
    WORKSTREAMS_DIR.mkdir(exist_ok=True, parents=True)

    yaml_files: List[Path] = list(PLANS_DIR.glob("*.yml")) + list(
        PLANS_DIR.glob("*.yaml")
    )
