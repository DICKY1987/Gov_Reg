# DOC_ID: DOC-SCRIPT-1128
# Alert Rule Identifier Troubleshooting Runbook
...
**Resolution:**
1. Check ID format against regex: `^ALERT-([A-Z]+)-([A-Z0-9-]+)-([0-9]{3,})$`
...
1. **Restore from backup:**
   ```bash
   cp RUNTIME/alerts/ALERT_REGISTRY.yaml.backup RUNTIME/alerts/ALERT_REGISTRY.yaml
   ```

2. **Rebuild from source:**
   ```bash
   python scripts/automation/alert_id_scanner.py --rebuild
   ```
...
# Check coverage
python scripts/automation/alert_id_scanner.py --report
```
...
2. Review recent commits affecting alert_id
