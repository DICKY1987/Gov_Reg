# DOC_ID: DOC-SCRIPT-1127
# Alert Rule Identifier Registry

**Type ID:** `alert_id`
**Classification:** minted
**Owner:** Monitoring
**Status:** Active

---

## Overview

Identifier for monitoring alert rules

**Format:** `ALERT-{SEVERITY}-{NAME}-{SEQ}`

## Categories

- **CRITICAL** - CRITICAL category
- **WARNING** - WARNING category
- **INFO** - INFO category

---

## Registry Structure

```yaml
alert_ids:
  - id: ALERT-<SEVERITY>-<NAME>-<SEQ>
    name: <human_readable_name>
    status: active
    created_utc: <ISO8601_timestamp>
    categories: [<category_list>]
```

---

## Usage

### Minting a New Alert Rule Identifier

1. Run the ID assigner: `python scripts/assign_alert_id.py`
2. Or manually add to registry following format
3. Validate with: `python scripts/validators/validate_alert_id.py`

---

## Validation

This registry is validated by:
- Format validator (regex: `^ALERT-([A-Z]+)-([A-Z0-9-]+)-([0-9]{3,})$`)
- Uniqueness validator
- Sync validator (registry ↔ filesystem)

---

## Automation

*No automation configured*

---

## References

- **ID Type Registry:** `RUNTIME/doc_id/SUB_DOC_ID/ID_TYPE_REGISTRY.yaml`
- **Governance:** `.ai/governance.md`
- **Universal Registry Schema:** `CONTEXT/docs/reference/UNIVERSAL_REGISTRY_SCHEMA_REFERENCE.md`

---

**Last Updated:** 2026-01-04
