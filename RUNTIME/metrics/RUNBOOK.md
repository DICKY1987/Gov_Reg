# DOC_ID: DOC-SCRIPT-1155
# Metric Identifier Troubleshooting Runbook
...
**Resolution:**
1. Check ID format against regex: `^METRIC-([A-Z0-9]+)-([A-Z0-9-]+)-([0-9]{3,})$`
...
1. **Restore from backup:**
   ```bash
   cp RUNTIME/metrics/METRIC_REGISTRY.yaml.backup RUNTIME/metrics/METRIC_REGISTRY.yaml
   ```

2. **Rebuild from source:**
   ```bash
   python scripts/automation/metric_id_scanner.py --rebuild
   ```
...
# Check coverage
python scripts/automation/metric_id_scanner.py --report
```
...
2. Review recent commits affecting metric_id
3. Contact: Metrics System
