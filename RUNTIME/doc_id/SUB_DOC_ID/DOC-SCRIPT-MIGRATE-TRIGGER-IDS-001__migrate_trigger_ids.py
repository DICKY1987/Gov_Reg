#!/usr/bin/env python3
# DOC_LINK: DOC-SCRIPT-MIGRATE-TRIGGER-IDS-001
# DOC_ID: DOC-SCRIPT-MIGRATE-TRIGGER-IDS-001
"""
doc_id: DOC-SCRIPT-MIGRATE-TRIGGER-IDS-001
Migration script for trigger_id format update
Migrates from TRIGGER-{CAT}-{SEQ} to TRIGGER-{CAT}-{NAME}-{SEQ}
"""
...
MIGRATION_MAP = SCRIPT_DIR / "trigger_id_migration_map.yaml"
REGISTRY_FILE = SCRIPT_DIR / "trigger_id" / "5_REGISTRY_DATA" / "TRG_ID_REGISTRY.yaml"
...
def backup_files():
    ...
    # Backup registry
    shutil.copy2(REGISTRY_FILE, backup_path / "TRG_ID_REGISTRY.yaml")
...
def scan_all_references(mappings):
    """Scan for all references to trigger IDs"""
...
def update_registry(mappings, dry_run=True):
    ...
    # Update each trigger entry
    for trigger in registry.get('triggers', []):
        old_id = trigger.get('trigger_id')
        ...
        if mapping:
            ...
            if not dry_run:
                trigger['trigger_id'] = mapping['new_id']
...
def rollback(backup_path):
    ...
    backup_registry = backup_path / "TRG_ID_REGISTRY.yaml"
...
def main():
    parser = argparse.ArgumentParser(description='Migrate trigger_id format')
    ...
    print(f"\n{'='*60}")
    print(f"TRIGGER_ID MIGRATION - {mode}")
    print(f"{'='*60}")
    ...
    print(f"\nMigrating {len(mappings)} trigger IDs")
    ...
    if dry_run:
        ...
        print("\nTo execute: python migrate_trigger_ids.py --execute")
    else:
        print("MIGRATION COMPLETE")
        print(f"Backup saved at: {backup_path}")
        print(f"To rollback: python migrate_trigger_ids.py --rollback {backup_path.name}")
    print("="*60)
