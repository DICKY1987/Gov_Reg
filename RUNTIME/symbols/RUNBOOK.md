# DOC_ID: DOC-SCRIPT-1173
# Code Symbol Identifier Troubleshooting Runbook
...
**Resolution:**
1. Check ID format against regex: `^SYM-[0-9a-f]{8}-[0-9a-f]{8}-[0-9a-f]{8}$`
...
**Diagnosis:**
```bash
python scripts/automation/symbol_id_scanner.py
```
...
1. **Restore from backup:**
   ```bash
   cp RUNTIME/symbols/SYMBOL_REGISTRY.yaml.backup RUNTIME/symbols/SYMBOL_REGISTRY.yaml
   ```

2. **Rebuild from source:**
   ```bash
   python scripts/automation/symbol_id_scanner.py --rebuild
   ```
...
# Check coverage
python scripts/automation/symbol_id_scanner.py --report
```
...
2. Review recent commits affecting symbol_id
