# DOC_ID: DOC-SCRIPT-1152
# Hook Identifier Troubleshooting Runbook
...
**Resolution:**
1. Check ID format against regex: `^HOOK-([A-Z0-9]+)-([A-Z0-9-]+)-([0-9]{3,})$`
...
1. **Restore from backup:**
   ```bash
   cp RUNTIME/hooks/HOOK_REGISTRY.yaml.backup RUNTIME/hooks/HOOK_REGISTRY.yaml
   ```

2. **Rebuild from source:**
   ```bash
   python scripts/automation/hook_id_scanner.py --rebuild
   ```
...
# Check coverage
python scripts/automation/hook_id_scanner.py --report
```
...
2. Review recent commits affecting hook_id
3. Contact: Hook Framework
