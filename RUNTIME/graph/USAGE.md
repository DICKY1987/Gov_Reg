# DOC_ID: DOC-SCRIPT-1150
# Graph Edge Identifier Usage Guide

**Type ID:** `graph_edge_id`
**Classification:** derived

---

## Quick Start

### Deriving IDs

IDs are automatically derived from source data.

**Derivation Rule:** Derived from edge endpoints and properties hash

```python
from scripts.automation.graph_edge_id_assigner import derive_id

source_data = {"key": "value"}
derived_id = derive_id(source_data)
```

---

## Validation

Validate the registry:

```bash
python scripts/validators/validate_graph_edge_id.py
```

---

## Registry Lookup

View all assigned IDs:

```bash
cat RUNTIME/graph/EDGE_REGISTRY.yaml
```

---

## Integration

### Pre-commit Hook

Automatically validates on commit (if enabled in automation).

### CI/CD Pipeline

Add to `.github/workflows/`:

```yaml
- name: Validate Graph Edge Identifier
  run: python scripts/validators/validate_graph_edge_id.py
```

---

**Last Updated:** 2026-01-04
