# DOC_ID: DOC-SCRIPT-1161
# Pipeline Instance Identifier Troubleshooting Runbook
...
**Resolution:**
1. Check ID format against regex: `^PIPELINE-[0-9A-HJKMNP-TV-Z]{26}$`
...
**Diagnosis:**
```bash
python scripts/automation/pipeline_id_scanner.py
```
...
1. **Restore from backup:**
   ```bash
   cp RUNTIME/pipelines/PIPELINE_REGISTRY.yaml.backup RUNTIME/pipelines/PIPELINE_REGISTRY.yaml
   ```

2. **Rebuild from source:**
   ```bash
   python scripts/automation/pipeline_id_scanner.py --rebuild
   ```
...
# Check coverage
python scripts/automation/pipeline_id_scanner.py --report
```
...
2. Review recent commits affecting pipeline_id
3. Contact: Pipeline Execution
