#!/usr/bin/env python3
# DOC_LINK: DOC-SCRIPT-TOOLS-PFA-VALIDATE-PATTERNS-SCHEMA-956
"""
Pattern Automation System Process Steps Schema Validator

Validates PATTERNS_PROCESS_STEPS_SCHEMA.yaml against standardized schema
requirements for pattern discovery, execution, and self-healing.
...
"""
DOC_ID: DOC-SCRIPT-TOOLS-PFA-VALIDATE-PATTERNS-SCHEMA-956
...
class PatternsValidator:
    """Validates Pattern Automation System process steps schema."""
    ...
    def validate_meta_section(self) -> None:
        ...
        required_meta = {"doc_id", "version", "last_updated", "description", "state_machine"}
    ...
    def validate_step(self, step: Dict[str, Any], phase_id: str) -> None:
        ...
        if "inputs" in step and not isinstance(step["inputs"], list):
            self.errors.append(
                f"Step '{step.get('step_id')}' inputs must be a list"
            )

        if "expected_outputs" in step and not isinstance(step["expected_outputs"], list):
            self.errors.append(
    ...
    def validate_guardrails(self) -> None:
        ...
        for gc in guardrails:
            required = {"checkpoint_id", "phase", "step_id", "description", "validation_logic", "on_fail"}
    ...
    def validate_state_machine_transitions(self) -> None:
        ...
        for from_state, to_states in transitions.items():
            ...
            for to_state in to_states:
                if to_state not in phases and to_state != "FAILED":
                    self.errors.append(
    ...
    def validate(self) -> bool:
        ...
        print("Validating Pattern Automation System Process Steps Schema...\n")
    ...
    def print_report(self) -> None:
        ...
        print("=" * 80)
        print("PATTERN AUTOMATION SYSTEM SCHEMA VALIDATION REPORT")
        print("=" * 80)
        ...
        if "meta" in self.schema:
            ...
            if "automation_levels" in meta and "fully_automatic" in meta["automation_levels"]:
                auto_level = meta["automation_levels"]["fully_automatic"]
                if "accuracy" in auto_level:
                    print(f"  Detection Accuracy: {auto_level['accuracy']}")
                if "auto_approval_threshold" in auto_level:
                    print(f"  Auto-Approval Threshold: {auto_level['auto_approval_threshold']}")
            if "state_machine" in meta and "automation_mode" in meta["state_machine"]:
                print(f"  Automation Mode: {meta['state_machine']['automation_mode']}")
            print()
...
def main() -> int:
    ...
    schema_path = script_dir / "PATTERNS_PROCESS_STEPS_SCHEMA.yaml"

    if not schema_path.exists():
        print(f"ERROR: Schema file not found: {schema_path}")
        return 1

    validator = PatternsValidator(schema_path)
