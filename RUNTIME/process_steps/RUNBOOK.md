# DOC_ID: DOC-SCRIPT-1164
# Process Step Identifier Troubleshooting Runbook
...
**Resolution:**
1. Check ID format against regex: `^STEP-PH-[A-Z0-9-]+-[0-9]{3,}-[0-9]{2}$`
...
**Diagnosis:**
```bash
python scripts/automation/step_id_scanner.py
```
...
1. **Restore from backup:**
   ```bash
   cp RUNTIME/process_steps/STEP_REGISTRY.yaml.backup RUNTIME/process_steps/STEP_REGISTRY.yaml
   ```

2. **Rebuild from source:**
   ```bash
   python scripts/automation/step_id_scanner.py --rebuild
   ```
...
# Check coverage
python scripts/automation/step_id_scanner.py --report
```
...
2. Review recent commits affecting step_id
