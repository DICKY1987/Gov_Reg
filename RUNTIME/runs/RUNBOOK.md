# DOC_ID: DOC-SCRIPT-1170
# Run Identifier Troubleshooting Runbook
...
**Resolution:**
1. Check ID format against regex: `^RUN-[0-9A-HJKMNP-TV-Z]{26}$`
...
**Diagnosis:**
```bash
python scripts/automation/run_id_scanner.py
```
...
1. **Restore from backup:**
   ```bash
   cp RUNTIME/runs/RUN_REGISTRY.yaml.backup RUNTIME/runs/RUN_REGISTRY.yaml
   ```

2. **Rebuild from source:**
   ```bash
   python scripts/automation/run_id_scanner.py --rebuild
   ```
...
# Check coverage
python scripts/automation/run_id_scanner.py --report
```
...
2. Review recent commits affecting run_id
3. Contact: Execution Engine
