# DOC_ID: DOC-SCRIPT-1176
# Trace Identifier Troubleshooting Runbook
...
**Resolution:**
1. Check ID format against regex: `^TRACE-[0-9A-HJKMNP-TV-Z]{26}$`
...
**Diagnosis:**
```bash
python scripts/automation/trace_id_scanner.py
```
...
1. **Restore from backup:**
   ```bash
   cp RUNTIME/traces/TRACE_REGISTRY.yaml.backup RUNTIME/traces/TRACE_REGISTRY.yaml
   ```

2. **Rebuild from source:**
   ```bash
   python scripts/automation/trace_id_scanner.py --rebuild
   ```
...
# Check coverage
python scripts/automation/trace_id_scanner.py --report
```
...
2. Review recent commits affecting trace_id
