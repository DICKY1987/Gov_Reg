# DOC_ID: DOC-SCRIPT-1140
# Deployment Identifier Troubleshooting Runbook
...
**Resolution:**
1. Check ID format against regex: `^DEPLOY-[0-9A-HJKMNP-TV-Z]{26}$`
...
**Diagnosis:**
```bash
python scripts/automation/deployment_id_scanner.py
```
...
1. **Restore from backup:**
   ```bash
   cp RUNTIME/deployments/DEPLOY_REGISTRY.yaml.backup RUNTIME/deployments/DEPLOY_REGISTRY.yaml
   ```

2. **Rebuild from source:**
   ```bash
   python scripts/automation/deployment_id_scanner.py --rebuild
   ```
...
# Check coverage
python scripts/automation/deployment_id_scanner.py --report
```
...
2. Review recent commits affecting deployment_id
3. Contact: Deployment System
