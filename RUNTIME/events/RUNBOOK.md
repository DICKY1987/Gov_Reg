# DOC_ID: DOC-SCRIPT-1146
# Event Identifier Troubleshooting Runbook
...
**Resolution:**
1. Check ID format against regex: `^EVENT-[0-9A-HJKMNP-TV-Z]{26}$`
...
**Diagnosis:**
```bash
python scripts/automation/event_id_scanner.py
```
...
1. **Restore from backup:**
   ```bash
   cp RUNTIME/events/EVENT_REGISTRY.yaml.backup RUNTIME/events/EVENT_REGISTRY.yaml
   ```

2. **Rebuild from source:**
   ```bash
   python scripts/automation/event_id_scanner.py --rebuild
   ```
...
# Check coverage
python scripts/automation/event_id_scanner.py --report
```
...
2. Review recent commits affecting event_id
3. Contact: Event System
