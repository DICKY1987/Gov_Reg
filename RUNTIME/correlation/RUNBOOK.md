# DOC_ID: DOC-SCRIPT-1134
# Correlation Identifier Troubleshooting Runbook
...
**Resolution:**
1. Check ID format against regex: `^CORR-[0-9A-HJKMNP-TV-Z]{26}$`
...
**Diagnosis:**
```bash
python scripts/automation/correlation_id_scanner.py
```
...
1. **Restore from backup:**
   ```bash
   cp RUNTIME/correlation/CORR_REGISTRY.yaml.backup RUNTIME/correlation/CORR_REGISTRY.yaml
   ```

2. **Rebuild from source:**
   ```bash
   python scripts/automation/correlation_id_scanner.py --rebuild
   ```
...
# Check coverage
python scripts/automation/correlation_id_scanner.py --report
```
...
2. Review recent commits affecting correlation_id
3. Contact: Observability
